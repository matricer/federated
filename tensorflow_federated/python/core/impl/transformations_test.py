...
def _create_call_to_dummy_intrinsic(fn, arg, uri='dummy'):
  r"""Creates a to call a dummy intrinsic.
...
def _create_call_to_federated_apply(fn, arg):
...
def _create_call_to_sequence_map(fn, arg):
...
def _create_chained_call(fn, arg, n):
  ...
  py_typecheck.check_type(arg,
                          computation_building_blocks.ComputationBuildingBlock)
  type_utils.is_assignable_from(fn.parameter_type, fn.result.type_signature)
  for _ in range(n):
...
def _create_chained_call_to_federated_map(fn, arg, n):
  ...
  py_typecheck.check_type(arg,
                          computation_building_blocks.ComputationBuildingBlock)
  for _ in range(n):
    call = _create_call_to_federated_map(fn, arg)
    arg = call
  return call
...
def _create_lambda_to_cast(parameter_type, result_type):
  r"""Creates a lambda to casts from `parameter_type` to `result_type`.
...
class TransformationsTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ('federated_apply',
       ...
       _create_call_to_federated_apply),
      ('federated_map',
       intrinsic_defs.FEDERATED_MAP.uri,
       computation_types.FederatedType(tf.int32, placements.CLIENTS),
       _create_call_to_federated_map),
      ('sequence_map',
       intrinsic_defs.SEQUENCE_MAP.uri,
       computation_types.SequenceType(tf.int32),
       _create_call_to_sequence_map))
  # pyformat: enable
  ...
  def test_remove_mapped_or_applied_identity_removes_multiple_identities(self):
    ...
    call = _create_chained_call_to_federated_map(fn, arg, 2)
    comp = call
  ...
  def test_remove_mapped_or_applied_identity_does_not_remove_other_intrinsic(
    ...
    fn = _create_lambda_to_identity(tf.int32)
    arg = computation_building_blocks.Data('x', tf.int32)
    call = _create_call_to_dummy_intrinsic(fn, arg)
    comp = call
  ...
  def test_replace_chained_federated_maps_replaces_federated_maps_with_different_types(
      self):
    fn_1 = _create_lambda_to_cast(tf.int32, tf.float32)
    arg_type = computation_types.FederatedType(tf.int32, placements.CLIENTS)
    arg = computation_building_blocks.Reference('x', arg_type)
    call_1 = _create_call_to_federated_map(fn_1, arg)
    fn_2 = _create_lambda_to_identity(tf.float32)
    call_2 = _create_call_to_federated_map(fn_2, call_1)
    comp = call_2
  ...
  def test_replace_chained_federated_maps_replaces_multiple_federated_maps(
    ...
    arg_type = computation_types.FederatedType(tf.int32, placements.CLIENTS)
    arg = computation_building_blocks.Data('x', arg_type)
    call = _create_chained_call_to_federated_map(fn, arg, 3)
    comp = call
  ...
  def test_replace_chained_federated_maps_does_not_replace_separated_federated_maps(
    ...
    call_2 = _create_call_to_federated_map(fn_2, block)
    comp = call_2
